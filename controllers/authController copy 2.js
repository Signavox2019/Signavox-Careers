...
exports.registerCandidate = async (req, res) => {
  try {
    let {
      ...
      currentAddress,
      socialLinks = {},
      role: inputRole,        // ✅ READ role from input
      team: inputTeam         // ✅ READ team from input
    } = req.body;
    ...
    if (typeof certifications === 'string') {
      try { certifications = JSON.parse(certifications); }
      catch { certifications = []; }
    }

    if (typeof socialLinks === 'string') {
      try { socialLinks = JSON.parse(socialLinks); }
      catch { socialLinks = {}; }
    }
    ...
    if (experienced && (!experiences || experiences.length === 0)) {
      return res.status(400).json({
        message: 'Experience details required for experienced candidates'
      });
    }
    ...
    if (req.files) {
      if (req.files['resume']) {
        resume = req.files['resume'][0].location || req.files['resume'][0].path;
      }
    ...
    // -----------------------------
    // ROLE & TEAM LOGIC (FIXED)
    // -----------------------------
    const ADMIN_EMAIL = 'khaja.rahiman@signavoxtechnologies.com';

    let role = 'candidate';
    let team = 'none';

    // If input role is provided and valid
    if (inputRole && ['candidate', 'recruiter', 'admin'].includes(inputRole)) {
      role = inputRole;
    }

    // If input team is provided and valid
    if (inputTeam && ['technical', 'marketing', 'finance', 'none'].includes(inputTeam)) {
      team = inputTeam;
    }

    // 🔒 FORCE ADMIN BY EMAIL (FINAL RULE)
    ...
    await sendMail({
      to: email,
      subject: 'SignaVox Careers - Account Created',
      html
    });
